...
def deletebysearch():
    ...
    if(keys!=''):
        print(keys)
        print(type(keys))
        sqliteConnection = sqlite3.connect(os.path.abspath(os.curdir)+'\hotfrogdetails.db')
        ...
        query = "DELETE FROM detail WHERE find LIKE '%{}%'".format(keys)
...
def view():
    ...
    findstriped = find.strip()
    nearstriped = near.strip()

    if(findstriped!='' and nearstriped!=''):
        # cur.execute("select * from detail where country")
        cur.execute("SELECT * FROM detail WHERE find=? and near=? ", (findstriped,nearstriped,))
        rows = cur.fetchall()
        if(len(rows)==0):
            cur.execute("select * from detail")
            rows = cur.fetchall()
...
# @app.route('/download')
# def download_file():
#     return send_file(os.path.abspath(os.curdir)+'\pages.txt',as_attachment=True)
    # csv = 'foo,bar,baz\nhai,bai,crai\n'
    # response = make_response(csv)
    # cd = 'attachment; filename=details.csv'
